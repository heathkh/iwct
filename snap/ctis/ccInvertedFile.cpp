...
float ivFile::weightVal(float val, ivWord& word, ivDoc& doc, ivFile::Weight wt)
{
  switch(wt)
  {
    ...
    case WEIGHT_TFIDF:
      val /= doc.ntokens ? doc.ntokens : 1;
      val *= log2(this->ndocs / (word.ndocs + CTIS_EPS));
      break;
  }
  return val;
}
...
float ivFile::normVal(float val, ivDoc& doc, ivFile::Norm norm)
{
  ...
  {
    case NORM_L0:
      val /= (CTIS_EPS + doc.norml0);
      break;
    case NORM_L1:
      val /= (CTIS_EPS + doc.norml1);
      break;
    case NORM_L2:
      val /= (CTIS_EPS + sqrt(doc.norml2));
      break;
  }
  return val;
}
...
//------------------------------------------------------------------------
template <class T>
void ivSearchFileCustom(ivFile& iv, T* wlabel, uint ntokens,
        ivFile::Weight weight, ivFile::Norm norm, ivFile::Dist dist,
        bool overlapOnly, uint k, ivNodeList& scorelist)
{
  //return if empty
...
//   cout << "we have " << ndocs << " docs" << endl;
  ...
  for (uint d=0; d<ndocs; d++)
  {
    ...
    ivSearchFileCustom(iv, doc.first, doc.second, weight, norm, dist, overlapOnly,
            k, ttscorelist);
...
//  os.write((char*)&ivdoc.nwords, sizeof(ivdoc.nwords));f
  return os;
}
...
//------------------------------------------------------------------------
bool ivFile::load(string filename)
{
  //open the file for opening
  ifstream inf;
...
#define IVSEARCHFILE2(T) \
  template void ivSearchFileCustom(ivFile&, T*, uint, ivFile::Weight, \
        ivFile::Norm, ivFile::Dist, bool, uint, ivNodeList&);

TEMPLATE(IVFILLFILE)
TEMPLATE(IVFILLFILE2)
TEMPLATE(IVSEARCHFILE)
TEMPLATE(IVSEARCHFILE2)
