  ...
  namespace bloom_filters {
	...
	*this = tmp;
	...
	bits.resize(new_capacity);
      ...
      void to_block_range(BlockOutputIterator result) {
        boost::to_block_range(bits, result);
      }

      template <typename BlockIterator>
      void from_block_range(int num_bits, BlockIterator first, BlockIterator last) {
        bits.clear();
        bits.resize(num_bits);
        boost::from_block_range(first, last, bits);
      }
