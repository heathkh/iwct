...
void CheckIsValid(const orsa::matrix<double>& mat);
...
class OrsaModel {
 public:
  typedef orsa::vector<double> Vec;
  typedef orsa::matrix<double> Mat;
  typedef Mat Model;
  ...
  double orsa(std::vector<size_t> &vec_inliers,
              size_t nIter = 1000,
              double desired_precision = -1, // -1 is magic value for don't care
...
private:
  ...
  void bestNFA(const std::vector<ErrorIndex>& e,
                     double loge0, double maxThreshold,
                     const std::vector<float> & vec_logc_n,
                     const std::vector<float> & vec_logc_k, OrsaModel::ErrorIndex* bestIndex) const;
