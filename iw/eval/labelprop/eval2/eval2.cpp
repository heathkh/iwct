 ...
 dataset_(tide_uri)
{
  ...
  for (int vertex_index=0; vertex_index < irg_.vertex_size(); ++vertex_index){
    ...
    if (!dataset_.ImageIsPrimary(v.image_id())) continue;
...
bool EvaluationRunner::Run(const Params& params, Result* result) {
  CHECK(result);
  result->Clear();
  typedef boost::unordered_map <uint32, LabelPredictions> RegionIdToLabelPredictions;
  typedef std::vector<RegionIdToLabelPredictions> ConfusionMatrixRegionLabelPredictionsRow;
  typedef std::vector<ConfusionMatrixRegionLabelPredictionsRow> ConfusionMatrixRegionLabelPredictions;

  ConfusionMatrixRegionLabelPredictions confusion_matrix_region_label_predictions;
  for (int i=0; i < num_labels_; ++i){
    confusion_matrix_region_label_predictions.push_back(ConfusionMatrixRegionLabelPredictionsRow(num_labels_));
  }
  ...
  for (uint32 trial_index=0; trial_index < params.num_trials(); ++trial_index) {
    ...
    dataset_.GenerateTrial(params.num_training_images(), params.frac_aux_images(), &trial);
    ...
    BOOST_FOREACH(const tide::ExperimentTrial_ImageLabelPair& p, trial.testing()){
      ...
      // For each GT region in test image
      BOOST_FOREACH(uint32 gt_region_index, primaryphotoid_to_gtregions_[p.image_id()]){
        uint32 predicted_label_index = unknown_label_index;

        // Find the cluster region with largest overlap and assign it that label (or leave unknown if no cluster regions overlap it)
        const vector<uint32>& overlap_edges = gtregion_overlap_edges_[gt_region_index];
        if (overlap_edges.size()){
          const ImageRegionGraph_Edge& edge = irg_.edge(overlap_edges[0]);
          uint32 best_overlap_cluster_region_index = edge.dst(); // sorted so that first is the cluster region with largest overlap
          CHECK_EQ(edge.src(), gt_region_index); // sanity check
          CHECK_NE(edge.dst(), gt_region_index); // sanity check
          predicted_label_index = GetBestLabel(R, best_overlap_cluster_region_index, unknown_label_index, params.min_score_threshold(), params.uniqueness_threshold());
          confusion_matrix_region_label_predictions[predicted_label_index][actual_label_index][best_overlap_cluster_region_index].AddPredictions(R, best_overlap_cluster_region_index);
        }
        ...
        //confusion_matrix_region_label_predictions[predicted_label_index][actual_label_index][gt_region_index].AddPredictions(R, best_overlap_cluster_region_index);
      }
      // For each unmarked cluster region in test image
      actual_label_index = unknown_label_index;
      BOOST_FOREACH(uint32 region_index, primaryphotoid_to_unmarked_clusterregions_[p.image_id()]){
        uint32 predicted_label_index = GetBestLabel(R, region_index, unknown_label_index, params.min_score_threshold(), params.uniqueness_threshold());
        trial_confusion_matrix(predicted_label_index, actual_label_index) += 1.0;
        confusion_matrix_region_label_predictions[predicted_label_index][actual_label_index][region_index].AddPredictions(R, region_index);
      }
    }
  ...
  for (int i=0; i < num_labels_; ++i){
    ...
    for (int j=0; j < num_labels_; ++j){
      ...
      const RegionIdToLabelPredictions& regionid_to_labelpredictions = confusion_matrix_region_label_predictions[i][j];
      BOOST_FOREACH(RegionIdToLabelPredictions::value_type v, regionid_to_labelpredictions){
...
void EvaluationRunner::CreateSimilarityMatrix(const tide::ExperimentTrial& trial, SparseMatrix* S) {
  //int num_regions = irg_.vertex_size();
