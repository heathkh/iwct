...
from iw.vis.export import tide_image_graph
...
#    self.image_graph_uri = '%s/7192be9ebd89da866039ed7cbb3d6011/cbir/494617b7c40f9dce9faa2f15e3de0041/itergraph/a6a2155b8d4f91f6114cebb36efd1d5a/image_graph.pert' % (base_uri)
  ...
  def ExportGraphViewer(self):
    ...
    tide_data = {}
    num_tide_objects = len(self.image_graph.tide.objectid_to_object)
    colors =  visutil.GenerateDistinctColorsCSS(num_tide_objects)
    for i, obj in enumerate(self.image_graph.tide.objectid_to_object.itervalues()):
      tide_data[obj.id] =  {'name' :  obj.name, 'color' : colors[i]}
    env = Environment(loader=PackageLoader('iw.vis.export', '.'))
  ...
  def ExportClusterSummary(self):
    ...
    cluster_membership = []    
    for cluster in self.image_graph.GetClustering():
    ...
    for cluster_members in cluster_membership:
      ...
      cluster_info.append((cluster_size, central_vertex['image_id'])) 
    env = Environment(loader=PackageLoader('iw.vis.export', '.'))
  ...
  def ExportGraphJson(self):
    ...
    for i, vertex in enumerate(g.vs):
      ...
      neighbors = [n['image_id'] for n in vertex.neighbors()]
      ...
      filename = '%s/%d.json' % (base_path, image_id)
      open(filename, 'w').write(json.dumps(data))
      progress.update(i)
    return
  
  def ExportImages(self):
    image_size_cache_filename = './images/size_cache.pickle'
    ...
    for i, (k,v) in enumerate(reader):      
      ...
      filename = './images/%d.jpg' % (image_id)
      f = open(filename, 'wb')
  ...
  def ExportMatchesJson(self):
    ...
    for i, (k,v) in enumerate(reader):
      ...
      if not match_result.matches:
        continue
      filename = './matches/%d_%d.json' % (image_a_id, image_b_id)
      f = open(filename, 'w')
      data = {}
      data['image_a'] = image_a_id
      data['image_b'] = image_b_id
